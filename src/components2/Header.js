...
export default class Header extends Component {
  ...
  //   context.methods.setAppState( { currentStep: step+1 } );
  // }
  render() {
    const { context } = this.props;

    const list = context.state.steps.map((field, key) =>
      <div key={key} className={context.state.currentStep === field.id ? 'header__item header__item_' + field.id + ' active' : 'header__item header__item_' + field.id}>
        {field.name}
      </div>
    );
    ...
    return (
      <div className="header">
        <div className="header__item header__title">
          <div className='header__ico'><img src={context.state.machine_names[context.state.currentAvto-1].image}/></div>
...
// {context.state.currentStep === 1 ? "header__item active" : "header__item" }
