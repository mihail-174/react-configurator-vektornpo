...
export default class ItemCheckBox extends Component {
    ...
    render() {
        ...
        const { context } = this.props;
        const state = context.state;
        const field = state.car['step_' + state.currentStep + '_field_' + itemGroupId].values.checkbox;

        const list = field.map((checkboxGroup, key) =>
            <div key={key} className={
                ...
                <label className="item__label" htmlFor={itemGroupName + "-" + key}>
                    {checkboxGroup.ico && <div className='item__image'><img src={require('../img/step-ico/' + checkboxGroup.url)} alt='' /></div>}
                    <div className='item__text'>
                        <div className='item__name'>{checkboxGroup.name}</div>
                        {checkboxGroup.subName && <div className='item__subname'>{checkboxGroup.subName}</div> }
                    </div>
                </label>
                { checkboxGroup.tooltip && <Tooltip context={context} itemGroupName={itemGroupName} itemGroupId={itemGroupId} itemId={key} /> }
            </div>
      );
